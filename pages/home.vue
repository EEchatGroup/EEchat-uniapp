  ...
  <view id="home">
    <view class="head">
      ...
      <view class="headRight">
        <image
          src="../static/more-operations.png"
          mode=""
          class="headIcon"
...
import { newest_seq, pull_msg, add_blacklist } from "../api";
export default {
  data() {
    return {
      sessionList: [],
  ...
  methods: {
    ...
    async getInfoList() {
      ...
      await newest_seq(parameter).then(async (res) => {
        ...
        console.log(
          uni.getStorageSync(this.userInfo.address + "latestSeq"),
          "qqqqqq2222"
        );
        ...
        if (
          res.data.data.seq >= 1 &&
          uni.getStorageSync(that.userInfo.address + "latestSeq").length == 0
        ) {
          console.log("老用户 换机登录");
          uni.setStorageSync(
            that.userInfo.address + "latestSeq",
            res.data.data.seq
          );
          parameter2.data.seqBegin = 1;
          parameter2.data.seqEnd = uni.getStorageSync(
            that.userInfo.address + "latestSeq"
          );
          ...
          console.log(
            uni.getStorageSync(that.userInfo.address + "latestSeq"),
            "换机登录seq"
          );
          ...
          that.sessionList = [];
          uni.setStorageSync(
            that.userInfo.address + "sessionList",
            that.sessionList
          );
          console.log("新用户 没消息");
          ...
          let localMessage = uni.getStorageSync(
          ...
          try {
            uni.setStorageSync(
              this.userInfo.address + "sessionList",
              this.sessionList
            );
          } catch (e) {
            // error
          ...
          parameter2.data.seqBegin = uni.getStorageSync(
            that.userInfo.address + "latestSeq"
          );
            ...
            for (let i = 0; i < newSingle.length; i++) {
              for (let t = 0; t < localMessage.length; t++) {
                if (localMessage[t].id == newSingle[i].id) {
                  localMessage[t].list = localMessage[t].list.concat(
                    newSingle[i].list
                  );
                  try {
                    uni.setStorageSync(
